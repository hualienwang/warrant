let
    ...
    SourceTable = Csv.Document(LatestCsv[Content], [Delimiter = ",", Columns = 18, Encoding = 65001, QuoteStyle = QuoteStyle.Csv]),
    ...
    CheckHeader = if List.Contains(Table.ColumnNames(SourceTable), "權證代號") then SourceTable
                    ...
                    let
                        ...
                        Promoted = Table.PromoteHeaders(Table.Skip(SourceTable, ActualSkip), [PromoteAllScalars = true])
                    in
                        Promoted,
