...
HTTP 402 means quota exceeded.

## How to Query

Use Python with `requests` and `pandas`.
...
If unsure which dataset to use, read the full dataset reference at `.claude/commands/finmind-references/datasets.md` for the complete list with column details and tier requirements.
...
Use **Chinese** for title, axis labels, legend, and annotations.

Default matplotlib may not include CJK glyphs. Before plotting, register a Chinese font when available:
...
If the font does not exist, check available Chinese fonts first. If needed, use `fonts-wqy-zenhei` or `fonts-noto-cjk` with `/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc`.

## Multi-Step Query Patterns

Complex questions often require multiple API calls.
...
1. Based on the user's request, determine the intent and map to the right dataset(s) using the intent mapping above.
